...
Define how the controller compiles workflow work using short-lived,
stateless variable resolvers while retaining the durable source data,
resolver recipes, resolved snapshots, and compilation outputs needed for
later stages, retries, audit, and controller restart.

This epic owns the compilation-resolution boundary for submitted workflows
and later ready steps. It turns workflow definitions, project/configuration
scopes, submission overrides, generated runtime values, predecessor outputs,
step bindings, and work-item bindings into concrete compiled work artifacts.
...
remarkably similar. They should share resolver recipe construction,
scope assembly, validation, provenance capture, fingerprinting, and compiled
artifact publication wherever their lifecycle inputs overlap.
...
through `internal/workflow`, and appends every generated work item to the
pending queue. That creates two related problems:
...
The target model separates durable lifecycle state from ephemeral resolution.
A resolver is built for one named compilation event, resolves the required
values, produces validated outputs, and is discarded. The controller persists
or retains the source scopes, generated runtime scopes, resolved snapshots,
provenance, fingerprints, and compiled artifacts needed for later decisions.
...
Names are intentionally provisional. The important design point is that the
shared operation receives explicit lifecycle inputs and returns explicit
compilation outputs. It does not retain a resolver and does not hide workflow
state inside a controller-global mutable object.

The shared operation should cover, where possible:

- namespace precedence and scope assembly;
- generated runtime scope construction;
- predecessor-output scope construction;
- step-local and work-item-local binding construction;
- required-value lookup and validation;
- fan-out binding expansion;
- resolved parameter snapshot capture;
- redacted provenance capture;
- fingerprint input collection;
- deterministic compiled-work identity construction;
- compilation diagnostics;
- idempotency keys for already-compiled stages.

The lifecycle-specific layer should own only the differences:

| Concern | Case 2 Submission | Case 3 Ready Step |
|---|---|---|
| Run identity | Generate new run identity | Load existing run identity |
| Definition source | Load and pin selected definitions | Reload from exact run-pinned identity or active pin |
| Environment capture | Capture declared values | Reload captured values/protected references |
| Predecessor outputs | Usually unavailable | Load completed typed outputs |
| Stage selection | Initial ready stage | Newly ready stage |
| Transaction purpose | Accept new run and publish initial work | Extend existing run with downstream work |

## Relationship To Other Epics

- `dependency-aware-workflows` should consume this epic. It decides when a stage
  is dependency-ready and calls the compilation boundary to publish work for
  that stage.
- `workflow-execution-persistence` owns the durable database-backed run, step,
  work-item, attempt, output, configuration-snapshot, and restart state needed
  to make this model authoritative across restart.
...
1. Should this epic be named `workflow-compilation-resolution` instead of the
   current folder spelling `workflow-complilation-resolution`?
2. Is a resolver recipe a first-class Go struct, a persisted JSON document, a set
   of database records, or simply a reconstructable controller contract?
3. Which parts of the resolver recipe must be durable before
   `workflow-execution-persistence` exists?
4. What is the minimum first implementation that improves correctness without
   prematurely designing the final database schema?
5. Does Case 2 persist source expressions for every later stage, or does it
   retain the entire normalized workflow definition and reconstruct future
   expressions from that definition?
6. Should resolved provenance be stored for every consumed value, only for
   non-sensitive values, only in debug mode, or only in failure diagnostics?
7. What exact provenance shape is needed to explain precedence without leaking
   sensitive values?
8. Which resolved values participate in workflow, step, work-item, input,
   output, and code-version fingerprints?
9. Should compilation produce work items directly, or should it first produce an
   intermediate compiled-stage representation that persistence later publishes?
10. Does fan-out expansion belong inside the shared compilation operation, or is
    fan-out a separate compiler phase that consumes a resolver?
11. What is the exact typed output shape made available from predecessor steps?
12. What namespace exposes predecessor outputs to downstream expressions?
13. Are predecessor outputs read as `workflow.step[index]`, a generated runtime
    scope, a workflow read-only scope, or another typed scope?
14. What happens when a downstream expression references a future or unavailable
    step output?
15. Should empty fan-out create no work item and immediately complete the stage,
    or create a deterministic skipped/no-op work item?
16. Which stage-level or step-level values may be recomputed during Case 3, and
    which must come only from the Case 2 run snapshot?
17. How should protected sensitive values captured at Case 2 be materialized for
    Case 3 without persisting plaintext?
18. Should compilation be allowed to read current controller operational metrics,
    or must those remain outside workflow semantics unless explicitly captured?
19. What is the boundary between compilation and database transaction ownership?
20. Can the shared compilation context builder be pure/in-memory, with callers
    responsible for loading and persisting state?
21. How should compilation failures be represented: submission rejection, blocked
    run, failed stage, or retryable controller error?
22. Can a ready-stage compilation be retried safely after a crash before its
    transaction commits?
23. What idempotency key uniquely identifies an already-compiled stage?
24. How should compilation versioning and schema evolution be recorded so older
    runs remain explainable?
25. Which pieces of compiled work are immutable after publication?
26. Can any compilation artifacts be cached, and if so what is the cache key and
    eviction policy?
27. How much of the existing `internal/workflow` compiler should be reused versus
    wrapped by a controller-side compilation boundary?
28. What is the eventual boundary for plugin-provided compile-time behavior, if
    plugins need to contribute declared outputs, package references, or parameter
    schemas?
29. How should this epic divide responsibilities with `workflow-execution-persistence`
    so neither document owns the same durable state twice?
30. Should `dependency-aware-workflows` be revised immediately after this epic is
