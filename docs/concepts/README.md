...
- [Work-Item Pause and Resume](dmtcp-work-item-checkpoint-resume/README.md) - Uses one controller lifecycle with per-work-item adapters. OS-009 implements and smoke-tests the explicit direct-CPython DMTCP adapter. OS-010 proves restart-on-resume for rclone 1.71.2 Google Drive single-file `copyto`, and OS-011 implements and real-supervisor-tests the shutdown-only native adapter: a replacement retransfers the full immutable object and reproduces exact output, while partial-prefix continuation remains unavailable.
