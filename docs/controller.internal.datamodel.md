...
with persisted running attempts one configured interval in which to report.

Two variables govern the initial recovery window:
...
`controller_config.worker_timeout_seconds` is a required positive integer in
the serialized controller JSON. A future duration type could replace the
unit-bearing name, but the initial type must make units unambiguous.

The controller also captures:

```text
runtime.controller_recovery_started_at
```

when the heartbeat/report endpoint becomes available. This is the timestamp
used for the restart recovery deadline:

```text
recovery_deadline = runtime.controller_recovery_started_at
                  + controller_config.worker_timeout_seconds
```
...
listener. Workers must receive the full reporting interval after they can
actually contact the replacement controller.
...
6. Reject new submissions and work claims during the recovery window
7. At the deadline, find running attempts with no accepted report in this epoch
8. Transactionally record each such attempt abandoned and requeue its work item
9. Reconcile ready stages and queued demand
10. Enter normal API mode
```
...
```text
worker_deadline = last_accepted_report_at
                + controller_config.worker_timeout_seconds
```

It does not continue using controller start time. Every accepted heartbeat
renews `last_accepted_report_at`. The configured timeout must be greater than
