...
- [Work-Item Pause and Resume](dmtcp-work-item-checkpoint-resume/README.md) - Uses one controller lifecycle with per-work-item adapters: DMTCP for direct R/Python interpreters, native continuation for rclone-backed work, and explicit application-level state for in-process Go operations. OS-003 proves the scoped RStan/CmdStanR path, OS-004 proves the scoped CPython 3.11/NumPy path with one Python descendant, and OS-005 implements the shared immutable resume-artifact manifest/reference model with typed strategy payloads.
