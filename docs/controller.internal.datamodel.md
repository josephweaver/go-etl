...
For the current controller, startup configuration has exactly three external
sources:
...
controller config JSON
    ...
    < generated read-only startup runtime values
```

Generated runtime values are controller outputs rather than an external
configuration source. They occupy the highest variable namespace so users
cannot override identities or captured timestamps owned by the controller.
