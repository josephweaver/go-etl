...
2. **Controller system environment.** Qualified `controller_env.*` references
   directly access matching variables from the controller process environment.
...
##### Controller environment accessor

`controller_env` is a direct accessor namespace over the controller process
environment. A qualified reference such as `${controller_env.DB_PASSWORD}`
reads the operating-system environment key `DB_PASSWORD`; no separate mapping
table or hard-coded key catalog is required.

The controller does not import or expose the entire process environment. A
resolver reads only keys that an expression references. Within one bounded
resolution, it reads each referenced key once and reuses that value so the
operation observes a consistent snapshot. A later resolver may observe a later
process-environment value if the process environment was explicitly changed.

Missing keys produce normal qualified-reference errors. Type conversion and
sensitivity propagation remain explicit variable-system concerns; diagnostics
must never expose sensitive material.
