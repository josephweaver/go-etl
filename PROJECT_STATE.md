...
- Geospatial worker plugins: [`docs/concepts/geospatial-worker-plugins/README.md`](docs/concepts/geospatial-worker-plugins/README.md). Operational slices `002-geospatial-operation-contract`, `003-raster-info-and-bounding-boxes`, `004-reproject-and-align-raster`, `005-stack-aligned-rasters`, `006-raster-pair-value-counts`, `007-crop-by-polygons`, `008-polygonize-raster`, and `009-aggregate-by-polygons` are implemented. The latest update adds `goet-geospatial` support for GDAL-backed `aggregate_by_polygons`, including vector polygon rasterization to temporary aligned zone rasters under the artifact root, explicit `all_touched` and value-nodata policies, CRS mismatch rejection, sorted `polygon_id,raster_value` CSV output with six-decimal proportions, metadata evidence, cleanup of temporary zone artifacts, and default plus GDAL-tagged test coverage.
