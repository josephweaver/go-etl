...
func TestControllerPolicyAuthorize(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "worker can confirm checkpoint",
			method: http.MethodPost,
			path:   "/work/checkpoint/confirm",
			role:   RoleWorker,
			want:   DecisionAllowed,
		},
		{
			name:   "admin can suspend from latest checkpoint",
			method: http.MethodPost,
			path:   "/work/checkpoint/suspend-latest",
			role:   RoleAdmin,
			want:   DecisionAllowed,
		},
		{
			name:   "client cannot confirm checkpoint",
			method: http.MethodPost,
			path:   "/work/checkpoint/confirm",
			role:   RoleClient,
			want:   DecisionDenied,
		...
		{
			name:   "checkpoint route rejects wrong method",
			method: http.MethodGet,
			path:   "/work/checkpoint/suspend-latest",
			role:   RoleAdmin,
		...
		{
			name:   "client cannot stop worker session",
			method: http.MethodPost,
			path:   "/workers/stop",
			role:   RoleClient,
			want:   DecisionDenied,
...
func TestControllerPolicyClassify(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "worker registration is protected",
			path: "/workers/register",
			want: RouteProtected,
		},
		{
			name: "checkpoint confirmation is protected",
			path: "/work/checkpoint/confirm",
			want: RouteProtected,
		},
		{
...
func TestControllerPolicyRouteRoleMatrix(t *testing.T) {
	...
	routes := []struct {
		...
		{name: "checkpoint confirmation", method: http.MethodPost, path: "/work/checkpoint/confirm", allowedRoles: []Role{RoleWorker, RoleAdmin}},
		{name: "checkpoint suspend latest", method: http.MethodPost, path: "/work/checkpoint/suspend-latest", allowedRoles: []Role{RoleWorker, RoleAdmin}},
		{name: "worker registration", method: http.MethodPost, path: "/workers/register", allowedRoles: []Role{RoleWorker, RoleAdmin}},
		{name: "worker heartbeat", method: http.MethodPost, path: "/workers/heartbeat", allowedRoles: []Role{RoleWorker, RoleAdmin}},
		{name: "worker stop", method: http.MethodPost, path: "/workers/stop", allowedRoles: []Role{RoleWorker, RoleAdmin}},
