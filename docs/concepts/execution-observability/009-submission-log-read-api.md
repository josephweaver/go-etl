...
- If `tail` is omitted, use the configured default from slice 002.
- If `tail` exceeds the configured maximum, return a client error or clamp to the maximum. Prefer a client error if that is easier to test clearly.
...
- The endpoint does not expose filesystem paths.
- The endpoint applies the default tail bound when `tail` is omitted.
- The endpoint validates invalid `tail` values.
- The endpoint applies the configured maximum tail bound.
