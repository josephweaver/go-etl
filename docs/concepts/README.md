...
- [Work-Item Pause and Resume](dmtcp-work-item-checkpoint-resume/README.md) - Uses one controller lifecycle with per-work-item adapters: DMTCP for direct R/Python interpreters, native continuation for rclone-backed work, and explicit application-level state for in-process Go operations. OS-003 and OS-004 prove scoped RStan/CmdStanR and CPython/NumPy feasibility, OS-005 through OS-007 implement the shared artifact, persistence, and transport contracts, and OS-008 implements the adapter-neutral worker drain and execution supervisor without enabling a production adapter.
